...
void print_header() {
    std::cout   << "========================================\n"
                << "                  CMAP                  \n"
                << "             Version: 0.1.4             \n"
                << " Copyright 2023 Maximilian A. C. Saller \n"
                << "========================================\n\n";
}

void write_observables() {
    std::ofstream ofile;
    ofile.open( "CPauli.out" );
    ...
    for ( int i=0; i<input.steps+1; ++i ) {
        ofile << std::scientific
        << std::setprecision(4) << std::setw(12) << obs.time[i]
        << std::setprecision(7) << std::setw(15) << obs.Cpop[i][0][0]
        << std::setprecision(7) << std::setw(15) << obs.Cpop[i][0][1]
        << std::setprecision(7) << std::setw(15) << obs.Cpop[i][1][0]
        << std::setprecision(7) << std::setw(15) << obs.Cpop[i][1][1]
        << "\n";
    }
    ofile.close();
}